...
/**
 * Resolve styling from a structured flag object { text, color, status }
 * or from legacy string values.
 */
function resolveFlagStyle(value) {
  // New structured format
  if (typeof value === 'object' && value !== null) {
    const color = value.color || value.status;
    if (color === 'green') return { type: 'pass', text: value.text };
    if (color === 'red') return { type: 'fail', text: value.text };
    if (color === 'yellow') return { type: 'skip', text: value.text };
    return { type: 'unavailable', text: value.text || 'Not available' };
  }
  // Legacy string format
  if (typeof value === 'string') {
    if (value.includes('✓')) return { type: 'pass', text: cleanText(value) };
    if (value.includes('✗')) return { type: 'fail', text: cleanText(value) };
    return { type: 'unavailable', text: cleanText(value) || 'Not available' };
  }
  return { type: 'unavailable', text: 'Not available' };
}
...
function AuditTrailSection({ auditTrail }) {
  ...
  return (
    <div className="mt-4 bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden">
      ...
      {entries.length === 0 ? (
        <p className="text-xs text-gray-400 italic p-5">Audit trail is not available for this job.</p>
      ) : (
        <ul className="divide-y divide-[#F1F5F9]">
          {entries.map((entry) => (
